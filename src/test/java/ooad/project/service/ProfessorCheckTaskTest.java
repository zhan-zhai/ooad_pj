...
/*
 * 测试监管局给一组专家发起监管任务，专家查看待完成任务，并完成抽检任务
 */
public class ProfessorCheckTaskTest {
    @Autowired
    ...
    public void testLaunchAndFinishSelfCheckTask() throws ParseException {
        ...
        Map<SpotCheckTask, List<ProductsType>> aUnfinishedTask = regulatoryTaskService.getUnfinishedTask(professorCheckTask);
        ...
        for (SpotCheckTask spotCheckTask:unfinishedTask.keySet()) {
            ...
            regulatoryTaskService.finishSpotCheckTask(professorCheckTask,spotCheckTask,spotCheckTaskCheckTime);
        }
        assert (professorCheckTask.isFinished());

        //此时任务全部完成，未完成任务集合为空
        assert (regulatoryTaskService.getUnfinishedTask(professorCheckTask).size()==0);
    }
    ...
    public void testHalfFinishedSelfTask() throws ParseException {//测试任务部分完成
        ...
        assert (unfinishedTaskEquals(regulatoryTaskService.getUnfinishedTask(professorCheckTask),aUnfinishedTask));
    }
